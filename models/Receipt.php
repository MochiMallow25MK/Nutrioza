...
class Receipt implements ICrudOperations, IReportable {
    ...
    public function __construct(int $receipt_id, int $location_id, string $supplier_name, string $product_name, int $quantity, string $unit, string $receipt_date, string $notes) {
    ...
    public function create(Database $db): bool {
        ...
        $stmt = $conn->prepare("INSERT INTO receipts (location_id, supplier_name, product_name, quantity, unit, receipt_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('isssisss', $this->location_id, $this->supplier_name, $this->product_name, $this->quantity, $this->unit, $this->receipt_date, $this->notes);
        $ok = $stmt->execute();
        $stmt->close();
        return $ok;
    }
    ...
    public function update(Database $db): bool {
        $conn = $db->getConnection();
        $stmt = $conn->prepare("UPDATE receipts SET location_id=?, supplier_name=?, product_name=?, quantity=?, unit=?, receipt_date=?, notes=? WHERE receipt_id=?");
        $stmt->bind_param('ississsi', $this->location_id, $this->supplier_name, $this->product_name, $this->quantity, $this->unit, $this->receipt_date, $this->notes, $this->receipt_id);
        $ok = $stmt->execute();
        $stmt->close();
        return $ok;
    }
